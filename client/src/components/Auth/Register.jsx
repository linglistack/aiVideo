...
const Register = ({ onRegister }) => {
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    try {
      ...
      setError(err.error || 'Failed to register with Google. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-black to-gray-900 py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-6">
        ...
        <div className="bg-tiktok-dark shadow-2xl rounded-xl overflow-hidden">
              ...
              <div>
                ...
                <div className="mt-1 relative">
                  <input
                    ...
                    onChange={handleChange}
                    className={`bg-gray-800 text-white rounded-xl w-full p-3 border 
                    ${confirmPassword && password !== confirmPassword ? 'border-red-500' : 'border-gray-700'} 
                    focus:border-tiktok-pink focus:ring-2 focus:ring-tiktok-pink focus:outline-none 
                    placeholder-gray-500`}
                    placeholder="Confirm your password"
                  />
                  {confirmPassword && password === confirmPassword && (
                    <IoCheckmarkCircleOutline className="absolute right-3 top-3 h-5 w-5 text-green-500" />
                  )}
                </div>
                {confirmPassword && password !== confirmPassword && (
                  <p className="mt-1 text-xs text-red-400 flex items-center">
                    <IoAlertCircleOutline className="mr-1 flex-shrink-0" />
          ...
          <div className="px-6 py-4 bg-[#0a0d14] sm:px-10">
            <p className="text-sm text-center text-gray-400">
              ...
              <Link to="/login" className="font-medium text-tiktok-pink hover:text-tiktok-blue transition-colors duration-200">
                Sign in instead
              </Link>
            </p>
          </div>
